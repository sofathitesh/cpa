...
$oid = makesafe($_REQUEST['campaign_id']);
...
http://YourSite.com/assets/postbacks/adworkmedia_postback.php

*/
$validIps = array('67.227.230.75', '"67.227.230.76');


$gid = safeGet($_GET['sid']);
$hash = safeGet($_GET['sid2']);
...
if(isset($_REQUEST['campaign_id']) && !empty($gid)){
	
	

  $offer_id = makesafe($_REQUEST['campaign_id']);
...
//get Offer Info
$sql2 = mysql_query("SELECT id FROM offers WHERE campaign_id = '$oid' AND active = 1 AND network = '$network' LIMIT 1");
...
mysql_query("UPDATE offer_process SET status = 1, date=NOW() WHERE code = '$hash' AND campaign_id = '$oid' AND network = '$network' AND gw_id = '$gid' AND status = 0 AND uid = $uid") or die("An error occured while setting lead status");
