...
$oid = makesafe($_REQUEST['oid']);
$status =  makesafe($_REQUEST['status']);
$subID = makesafe($_REQUEST['aff_sub']);
$network = 'Adgatemedia';
$password = 'CPNNETWORK';

if($_GET['aff_sub3'] != $password)
{
...
if(isset($_GET['aff_sub4']) && !empty($_GET['aff_sub4']) && $_GET['aff_sub4'] == 'gateway')
{
	
	require_once("../assets/CPANX_GWS_WIDGSPostbacks/adsgatemedia_postback.php");
    return;	
...
$uid = makesafe($_REQUEST['sub']);
if(empty($uid)){
$uid = makesafe($_REQUEST['aff_sub']);
}


$hash = makesafe($_REQUEST['sub2']);
if(empty($hash))
{
$hash = makesafe($_REQUEST['aff_sub2']);	
}
...
if($status == 0) //Status is not 1 it means offer not approved.
{
